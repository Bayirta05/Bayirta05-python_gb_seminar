...
'''lat_dict = {1: 'AEIOULNSTR', 2: 'DG', 3: 'BCMP', 4: 'FHVWY', 5: 'K', 8: 'JX', 10: 'QZ'}
cir_dict = {1: 'АВЕИНОРСТ', 2: 'ДКЛМПУ', 3: 'БГЁЬЯ', 4: 'Й,Ы', 5: 'ЖЗХЦЧ', 8: 'ШЭЮ', 10: 'ФЩЪ'}
text = input().upper()
if letter in cir_dict:
	print(sum([k for i in text for k, v in cir_dict.items() if i in v]))
else:
	print(sum([k for i in text for k, v in lat_dict.items() if i in v]))'''
